...
/**
 * Servlet pour effectuer un dépôt
 */
@WebServlet(name = "DepotServlet", urlPatterns = {"/depot"})
public class DepotServlet extends HttpServlet {
    
    private static final Logger LOGGER = Logger.getLogger(DepotServlet.class.getName());
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        context.setVariable("pageTitle", "Dépôt - Banque Premium");
            context.setVariable("currentPage", "depot");
        ...
        templateEngine.process("depot", context, response.getWriter());
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        ClientDTO client = (ClientDTO) session.getAttribute("client");
        LOGGER.info("Dépôt pour: " + client.getEmail());
        ...
        try {
            ...
            // Appeler le service distant pour effectuer le dépôt
            OperationServiceRemote operationService = EJBClientFactory.getOperationService();
            MouvementDTO mouvement = operationService.effectuerDepot(
                numeroCompte, 
                montant, 
                libelle != null && !libelle.trim().isEmpty() ? libelle : "Dépôt"
            );
            
            LOGGER.info("Dépôt effectué avec succès: " + mouvement.getReference());
            
            // Rediriger vers le dashboard
            response.sendRedirect(request.getContextPath() + "/dashboard?success=depot");
            
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Erreur lors du dépôt", e);
            context.setVariable("errorMessage", "Erreur lors du dépôt: " + e.getMessage());
            response.setContentType("text/html;charset=UTF-8");
            templateEngine.process("depot", context, response.getWriter());
        }
    }
